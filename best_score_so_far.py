        ...
        for i in range(m1, m2 + 1):
            for j in range(n1, n2 + 1):
                ...
                if candidate_coords in available_bw and available_bw[candidate_coords] > EPSILON:
                    ...
                    traffic_delay_score = (Tmax / (t+Tmax)) * (min(data['Q_rem'], available_bw[candidate_coords]) / Q_total)
                    score = 0.3 * min(data['Q_rem'], available_bw[candidate_coords]) * distance_score_multiplier / Q_total + 0.2 * traffic_delay_score
        ...
        for i in range(m1, m2 + 1):
            for j in range(n1, n2 + 1):
                ...
                if candidate_coords in available_bw and available_bw[candidate_coords] > EPSILON:
                    distance = abs(access_x - i) + abs(access_y - j)
                    distance_score_multiplier = 2 ** (-ALPHA * distance)
                    traffic_delay_score = (Tmax / (t+Tmax)) * (min(flow_data['Q_rem'], available_bw[candidate_coords]) / Q_total)
                    score = 0.3 * min(flow_data['Q_rem'], available_bw[candidate_coords]) * distance_score_multiplier / Q_total + 0.2 * traffic_delay_score
